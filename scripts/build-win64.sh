...
BTBN_SERIES="${2:?BtbN image series is required, e.g. 8.1 or 9.0}"
...
IMAGE="${BTBN_IMAGE:-ghcr.io/btbn/ffmpeg-builds/win64-gpl-${BTBN_SERIES}:latest}"
...
# Never trust the header version baked into the upstream image. Replace it
# explicitly with the legacy API baseline selected by this project.
...
# their date-based --extra-version. The image supplies all dependency flags.
...
# Compile-time validation. This does not replace the later hardware test on
# a Pascal card, but it catches accidental loss of NVENC in the binary.
...

{
  ...
  echo "NVENC API: 13.0"
  echo "BtbN build-system commit: ${BTBN_BUILD_COMMIT}"
...
# Useful dependency sanity check for a supposedly static Windows build.
{
